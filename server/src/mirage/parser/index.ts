...
for (let i = 0; i < ins.length; i++) {
  ...
  if (normalized) {
    ...
    const result = new SDCFormResponseService().validateSDCFormResponse(
      normalized,
      initialSDCFormResponse
    );

    fs.writeFileSync(SDCFRValidationOutputFileName, JSON.stringify(result));
  }
